defmodule AppstarWeb.HomeLive do
  ...
  def render(assigns) do
    ~H"""
    <h1 class="text-2xl">Appstar</h1>
    <.button type="button" phx-click={show_modal("new-post-modal")}>Create Post</.button>

    <div id="feed" phx-updates="stream" class="flex flex-col gap-2">
      <div
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Appstar.PubSub, "posts")
  ...
  def handle_event("save-post", %{"post" => post_params}, socket) do
    ...
    |> case do
      {:ok, post} ->
        ...
        Phoenix.PubSub.broadcast(Appstar.PubSub, "posts", {:new, Map.put(post, :user, user)})
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest = Path.join([:code.priv_dir(:appstar), "static", "uploads", Path.basename(path)])
      File.cp!(path, dest)
